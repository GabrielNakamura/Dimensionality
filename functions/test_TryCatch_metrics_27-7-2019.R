###TryCatch with all metrics
matrixM_error_all<- function(data){
  out<- tryCatch({
    ...
    #taxonomic metric
    rich<- rowSums(data)
    matrix_Mobs_nullMod1<- as.matrix(data.frame(PDfaith, mntd, PSV, Peve, FDfaith, FEve, FDiv, richness= rich))
